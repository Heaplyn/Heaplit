...
This specification provides complete architectural, structural, and operational details for **Anthropic Claude API Streaming Parser** within the Heaplit System.
...
namespace Heaplit.Core.Subsystems.ProtocolSpec
{
    public sealed class AnthropicClaudeAPIStreamingParserManager : IAsyncDisposable
    {
        ...
        public async Task<bool> ExecuteAnthropicClaudeAPIStreamingParserAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                Console.WriteLine("[HEAPLIT PROTOCOLSPEC] Executing Anthropic Claude API Streaming Parser operation...");
                ...
                if (NativeMethods.GetSystemTimes(out FILETIME idle, out FILETIME kernel, out FILETIME user))
                {
                    ...
                    if (totalSys > 0)
                    {
                        Console.WriteLine($"[HEAPLIT PROTOCOLSPEC] System Ticks Active: {totalSys}");
                    }
                }
